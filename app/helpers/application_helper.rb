module ApplicationHelper
  ...
  def build_validated_field(form_name, field, name, message, pattern, type, value)
    ...
    if type == 'password'
      html << password_field_tag("#{form_name}[#{field}]", value, :class => '', :required => '', :pattern => pattern, :placeholder => name)
    elsif type == 'file'
      html << file_field_tag("#{form_name}[#{field}]", :required => '', :pattern => pattern, :placeholder => name)
    elsif type == 'radio'
      html << radio_button_tag("#{form_name}[#{field}]", value, :class => '', :required => '', :pattern => pattern, :placeholder => name)
    elsif type == 'checkbox'
      html << check_box_tag("#{form_name}[#{field}]", value, :class => '', :required => '', :pattern => pattern, :placeholder => name)
    elsif type == 'text_area'
      html << text_area_tag("#{form_name}[#{field}]", value, :class => '', :required => '', :pattern => pattern, :placeholder => name,:rows=>1)
    elsif type == 'date_field'
      html << text_field_tag("#{form_name}[#{field}]", value, :class => 'date-input ', :required => '', :pattern => pattern, :placeholder => name)
    elsif type == 'date_only_field'
      html << text_field_tag("#{form_name}[#{field}]", value, :class => 'date-only-input', :required => '', :pattern => pattern, :placeholder => name)
    elsif type == 'time-field'
      html << text_field_tag("#{form_name}[#{field}]", value, :class => 'time-field', :required => '', :pattern => pattern, :placeholder => name)
    else
      html << text_field_tag("#{form_name}[#{field}]", value, :class => '', :required => '', :pattern => pattern, :placeholder => name)
    end
